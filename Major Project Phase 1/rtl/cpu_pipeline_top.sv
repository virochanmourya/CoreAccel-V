...
module cpu_pipeline_top (
    ...
    // ---- EX Stage: MAC Control Signals ----
    logic id_is_mac;            // From control unit: 1 if the instruction in ID/EX is a MAC_READ
    logic id_is_mac_clear;      // From control unit: 1 if the instruction in ID/EX is a MAC_CLEAR
    logic id_mac_to_reg;        // From control unit: 1 if the instruction in ID/EX is a MAC_READ (same as id_is_mac, but more explicit for mux control)
    logic is_mac_ex;            // Synchronized version of id_is_mac for use in EX stage
    logic is_mac_clear_ex;      // Synchronized version of id_is_mac_clear for use in EX stage
    logic mac_to_reg_ex;        // Synchronized version of id_mac_to_reg for use in EX stage
    ...
    logic        id_ex_freeze;
    logic        ex_mem_flush;
    logic        mac_busy_ex; // From the MAC unit in EX stage
    logic        if_id_write;
    logic        mac_stall_request; // NEW: The full stall window
    ...
    // 1. PC Logic: Freeze if Load-Use OR MAC is computing
    assign pc_write    = ~(stall | mac_stall_request) | branch_taken;

    // 2. IF/ID Logic: Freeze if Load-Use OR MAC is computing
    assign if_id_stall = (stall | mac_stall_request) & ~branch_taken;
    assign if_id_flush = branch_taken;

    // 3. ID/EX Logic: Flush on Load-Use. Freeze on MAC computation.
    assign id_ex_flush = stall | branch_taken;
    logic id_ex_stall;
    assign id_ex_stall = mac_stall_request;

    // 4. EX/MEM Logic: Flush on MAC computation to inject bubbles
    logic ex_mem_flush_sig;
    ...
    hazard_detection_unit u_hazard (
        ...
        .if_id_rs2      (id_rs2),
        .stall          (stall)
        // Note: Disconnect the other custom ports so the top-level logic handles the MAC stall perfectly.
    );
    ...
    id_ex_reg u_id_ex (
        ...
        .reset           (reset),
        .flush           (id_ex_flush),
        .stall           (id_ex_stall), // FIXED: The MAC is now trapped!
        ...
        .is_mac_in       (id_is_mac),
        .is_mac_clear_in (id_is_mac_clear),
        .mac_to_reg_in   (id_mac_to_reg),
        .is_mac_out      (is_mac_ex),
        .is_mac_clear_out(is_mac_clear_ex),
        .mac_to_reg_out  (mac_to_reg_ex),
    ...
    // --------------------------------------------------------
    // EX Stage: MAC Pulse Generator (Prevents Double-Firing)
    // --------------------------------------------------------
    logic mac_started_reg;
    
    // --------------------------------------------------------
    // EX Stage: MAC Pulse Generator & Stall Request
    // --------------------------------------------------------
    always_ff @(posedge clk) begin
        if (reset || !is_mac_ex) begin
            mac_started_reg <= 0;
        end else if (is_mac_ex && !mac_busy_ex) begin
            mac_started_reg <= 1;
        end
    end
    
    // Pulse to start the MAC (1 cycle only)
    logic mac_start_pulse;
    assign mac_start_pulse = is_mac_ex & ~mac_started_reg & ~mac_busy_ex;

    // NEW: The Full Stall Window
    // Stalls instantly when MAC hits EX, and holds until MAC is done.
    assign mac_stall_request = is_mac_ex & ~(mac_started_reg & ~mac_busy_ex);
    
    // --------------------------------------------------------
    // EX Stage: The DSP MAC Unit
    // --------------------------------------------------------
    logic [31:0] mac_result_ex;
    ...
    mac_unit dsp_core (
        ...
        // Feed it the hazard-safe forwarded operands!
        .operand_a   (forwarded_a), 
        .operand_b   (forwarded_b),
        
        .mac_result  (mac_result_ex),
        .mac_busy    (mac_busy_ex)
    );


    // --------------------------------------------------------
    // EX Stage: Final Output Routing
    // --------------------------------------------------------
    logic [31:0] ex_stage_result;
    
    // If the instruction is MAC_READ, route the MAC accumulator to the pipeline.
    // Otherwise, route the standard ALU result.
    assign ex_stage_result = mac_to_reg_ex ? mac_result_ex : alu_result;
    ...
    ex_mem_reg u_ex_mem (
        .clk             (clk),
        .reset           (reset),
        .flush           (ex_mem_flush_sig), // FIXED: Prevents garbage writes!
        ...
        // Data
        .alu_result_in   (ex_stage_result), // FIXED! Now the MAC data can flow through the pipeline correctly.
